    ...
    -->
        ...
        <table class="table table-striped table-hover tablesorter" id="fieldingTable">
            ...
            <tbody>
                <?php
                    // query the player details and bat speed tables and select the latest bat speed entry for each player, then order the table by bat speed
                    $query = "SELECT first_name, last_name, of3.throwing_speed as outfield_throwing_speed, if3.throwing_speed as infield_throwing_speed, c3.pop_time
                    FROM player_details

                    LEFT JOIN (
                        SELECT of1.*
                    	FROM outfielder_stats of1
                    	WHERE of1.date_stats_collected = (
                            SELECT MAX(of2.date_stats_collected)
                        	FROM outfielder_stats of2
                            WHERE of2.player_id = of1.player_id
                        )
                    ) of3 ON player_details.player_id = of3.player_id

                    LEFT JOIN (
                        SELECT if1.*
                    	FROM infielder_stats if1
                    	WHERE if1.date_stats_collected = (
                            SELECT MAX(if2.date_stats_collected)
                        	FROM infielder_stats if2
                            WHERE if2.player_id = if1.player_id
                        )
                    ) if3 ON player_details.player_id = if3.player_id
                    ...
                    while($row = $result->fetch_assoc()) {
                        ...
                        <tr>
                            <td><?php WriteName($row)?></td>
                            <td><?php echo FormatSpeed($row["outfield_throwing_speed"])?></td>
                            <td><?php echo FormatSpeed($row["infield_throwing_speed"])?></td>
